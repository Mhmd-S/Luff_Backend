...
export const getUserById = async (userId) => {
	const result = await User.findById(
		userId,
		'_id name dob bio gender orientation likedUsers blockedUsers verified profilePictures'
	).exec();
	return result;
};
...
export const getUserByEmail = async (email) => {
	const result = await User.findOne(
		{ email: email },
		'_id name dob bio gender orientation verified profilePictures'
	).exec();
	console.log(result);
...
export const addToRejectedUsers = async (userId, rejectedUserId) => {
	const result = await User.findByIdAndUpdate(userId, {
		$push: { rejectedUsers: rejectedUserId },
	}).exec();
	return result;
};
